...
' Subject: COP-1332
' Project: Chapter 11
' Description: Modification of Project 7.3 per instructions

Imports System.IO

Public Class MainForm

    ' Flag for checking dirty data
    Dim IsDirtyBoolean As Boolean
    ...
    Private Sub MainForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ' Open the file
            Dim YachtsStreamReader As StreamReader =
                New StreamReader("Yachts.txt")
            ' Read all elements into the list
            Do Until YachtsStreamReader.Peek = -1
                YachtTypeString = YachtsStreamReader.ReadLine()
                TypeComboBox.Items.Add(YachtTypeString)
            Loop
            ' Close the file
            YachtsStreamReader.Close()
    ...
    Private Sub MainForm_FormClosing(sender As Object, e As EventArgs) Handles MyBase.FormClosing

        ' Ask the user to save the file
        Dim ResponeDialogResult As DialogResult
        Dim MessageString As String = "Yacht list has changed. Save the list?"

        If IsDirtyBoolean Then
            ResponeDialogResult = MessageBox.Show(MessageString, "Yacht List Changed",
                MessageBoxButtons.YesNo, MessageBoxIcon.Question)

            If ResponeDialogResult = DialogResult.Yes Then
                SaveYachtListToolStripMenuItem_Click(sender, e)
            End If
        End If
    ...
    Private Sub SaveYachtListToolStripMenuItem_Click(ByVal sender As System.Object,
                ByVal e As System.EventArgs) Handles SaveYachtListToolStripMenuItem.Click

        ' Save the yacht list in a file
        Dim NumberItemsInteger As Integer

        ' Open the file
        Dim YachtsStreamWriter As StreamWriter = New StreamWriter("Yachts.txt", False)

        'Save the items in the file
        NumberItemsInteger = TypeComboBox.Items.Count - 1
        For IndexInteger As Integer = 0 To NumberItemsInteger
            YachtsStreamWriter.WriteLine(TypeComboBox.Items(IndexInteger))
        Next IndexInteger

        'Close the file
        YachtsStreamWriter.Close()
    ...
    Private Sub OKButton_Click(sender As Object, e As EventArgs) Handles OKButton.Click
        ...
        ' Declare variables and open reservations file
        Dim ReservationsStreamWriter As New StreamWriter("Res.txt", True)
        ...
        ' Add data to reservation file
        ReservationsStreamWriter.Write(DateTime.Now.ToString("yyyyMMdd") & ",")
        ReservationsStreamWriter.Write(PartyTextBox.Text & ",")
        ReservationsStreamWriter.Write(HoursTextBox.Text & ",")
        ReservationsStreamWriter.Write(TypeComboBox.Text & ",")
        ReservationsStreamWriter.Write(SizeComboBox.Text & ",")
        ReservationsStreamWriter.Write(PriceTextBox.Text & ",")
        ReservationsStreamWriter.Write(CommentsTextBox.Text & ",")
        ReservationsStreamWriter.WriteLine()
        ReservationsStreamWriter.Close()
    ...
    Private Sub ShowReservationsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ShowReservationsToolStripMenuItem.Click
